...
const DEFAULT_API_BASE = "https://ai-developer-api.onrender.com/api/v1";

/**
 * API base resolution:
 * - If `?api=https://.../api/v1` is in URL → use it (useful for testing)
 * - Else if user saved `api_base` in localStorage → use it
 * - Else use DEFAULT_API_BASE (Render backend)
 */
...
const API = {
  ...
  setBase(b){
    this.base = (b || "").replace(/\/+$/,"");
    localStorage.setItem("api_base", this.base);
  },
